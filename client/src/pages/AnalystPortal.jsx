...
function AnalystPortal() {
  ...
  useEffect(() => {
    axios
      .get(
        `https://newsdata.io{NEWS_API_KEY}&q=cybersecurity&language=en&size=6`
      )
      .then((res) => {
  ...
  return (
    <div className="p-6">
      {/* HEADER */}
      <h1 className="text-4xl text-green-500 font-bold mb-2">Analyst Portal</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="bg-gray-900 p-6 rounded-lg border border-green-500">
          ...
          <p className="text-4xl font-bold">{reportsLoading ? "..." : reports.length}</p>
        </div>
        <div className="bg-gray-900 p-6 rounded-lg border border-green-500">
          <h2 className="text-green-400 text-lg mb-1">Total Activities</h2>
          <p className="text-4xl font-bold">{auditLoading ? "..." : auditLogs.length}</p>
        </div>
        <div className="bg-gray-900 p-6 rounded-lg border border-green-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-10">
        ...
        <div className="bg-gray-900 p-6 rounded-lg border border-green-500">
          <h2 className="text-2xl text-green-400 font-bold mb-4">🕐 Activity Timeline</h2>
          {auditLoading && <p className="text-yellow-400">Loading activity...</p>}
          {!auditLoading && auditLogs.length === 0 && (
            <p className="text-gray-400">No activity yet. Run a scan or upload a report.</p>
          )}
          {!auditLoading && auditLogs.length > 0 && (
        ...
        <div className="bg-gray-900 p-6 rounded-lg border border-green-500">
          <h2 className="text-2xl text-green-400 font-bold mb-4">📁 Uploaded Reports</h2>
          {reportsLoading && <p className="text-yellow-400">Loading reports...</p>}
                ...
                <tbody>
                  {reports.map((report, index) => (
                    <tr key={report.id} className="border-b border-gray-800 hover:bg-gray-800">
                      ...
                      {/* REVERTED: Back to a normal text string */}
                      <td className="py-2 text-green-300 text-sm">{report.filename}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="bg-gray-900 p-6 rounded-lg border border-green-500">
        <h2 className="text-2xl text-green-400 font-bold mb-6">📡 Live Cyber Threat Intelligence</h2>
        {newsLoading && <p className="text-yellow-400">Fetching latest threat news...</p>}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {news.map((article, index) => (
              ...
              >
                ...
                <p className="text-green-400 text-xs mb-2 uppercase">{article.source_id || "Cybersecurity News"}</p>
