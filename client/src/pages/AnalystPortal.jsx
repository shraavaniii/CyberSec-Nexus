  ...
  return (
    <div className="p-2">
      ...
      <p className="text-blue-300 text-sm mb-8 opacity-70">
        ...
        {isAdmin && (
          <span className="ml-3 text-xs glass px-2 py-0.5 rounded-full font-bold text-blue-300"
      ...
      {isAdmin ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            ...
            <div className="glass rounded-2xl p-6"
              ...
              {!auditLoading && auditLogs.length === 0 && (
                <p className="text-blue-400 opacity-60 text-sm">No activity yet.</p>
              )}
              {!auditLoading && auditLogs.length > 0 && (
            ...
            <div className="glass rounded-2xl p-6"
              style={{ border: "1px solid rgba(0,150,255,0.2)" }}>
              <h2 className="text-xl font-bold text-white mb-4">📁 All Uploaded Reports</h2>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr style={{ borderBottom: "1px solid rgba(0,150,255,0.3)" }}>
                        ...
                        <th className="py-2 pr-4 text-blue-400 text-sm">By</th>
                    ...
                    <tbody>
                      {reports.map((report, index) => (
                        <tr key={report.id}
                          ...
                          <td className="py-2 pr-4 text-blue-300 text-xs">{report.uploaded_by || "—"}</td>
        ...
        <div className="glass rounded-2xl p-8 mb-8 text-center animate-fade-up"
          style={{ border: "1px solid rgba(255,150,0,0.3)", background: "rgba(255,150,0,0.06)" }}>
          <div className="text-6xl mb-4">🔒</div>
          <h2 className="text-yellow-400 font-bold text-2xl mb-3">Access Denied!</h2>
          <p className="text-blue-200 text-base mb-2">
            The Activity Timeline and Reports sections are restricted to <span className="text-yellow-400 font-bold">Admins only</span>.
          </p>
          <p className="text-blue-300 text-sm opacity-80">
            But you can still stay updated with the latest cybersecurity news below! 👇
          </p>
        </div>
      )}

      {/* LIVE CYBER NEWS — visible to EVERYONE */}
      <div className="glass rounded-2xl p-6"
        style={{ border: "1px solid rgba(0,150,255,0.2)" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {news.map((article, index) => (
              <a key={index} href={article.link} target="_blank" rel="noreferrer"
                className="block rounded-xl p-4 transition-all duration-300 hover:-translate-y-2"
